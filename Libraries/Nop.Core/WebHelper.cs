    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual bool IsCurrentConnectionSecured()
        {
            ...
            if (IsRequestAvailable(_httpContext))
            {
                ...
                else if (string.Equals(ServerVariables("HTTPS"), "on", StringComparison.OrdinalIgnoreCase))
                {
                    useSsl = true;
                }
                else
            ...
            if (string.Equals(ServerVariables("HTTPS"), "on", StringComparison.OrdinalIgnoreCase))
            {
                useSsl = true;
            }
        ...
        {
            string serverVariables = ServerVariables("HTTP_HOST");
            string result = "";
            if (useSsl)
            {
                if (serverVariables.Contains("www"))
                {
                    result = "http://" + serverVariables;
                }
                else
                {
                    result = "http://www." + serverVariables;
                }
                if (!result.EndsWith("/"))
                    result += "/";
            ...
            if (!string.IsNullOrEmpty(queryStringModification))
            {
                if (!string.IsNullOrEmpty(str))
                {
                    ...
                    foreach (string str5 in dictionary.Keys)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(dictionary[str5]))
                        {
                            builder.Append(str5);

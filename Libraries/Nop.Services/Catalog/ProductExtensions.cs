...
namespace Nop.Services.Catalog
{
    ...
    public static class ProductExtensions
    {
        ...
        public static string FormatStockMessage(this ProductVariant productVariant, ILocalizationService localizationService)
        {
            ...
            if (productVariant.DisplayStockAvailability)
            {
                ...
                {
                    var sum = productVariant.ProductVariantAttributeCombinations.Sum(x => x.StockQuantity);
                    switch (productVariant.BackorderMode)
                    {
                        case BackorderMode.NoBackorders:
                            {
                                if (sum>0)
                                {
                                    ...
                                    {
                                        //display "in stock" with stock quantity
                                        stockMessage = string.Format(localizationService.GetResource("Products.Availability.InStockWithQuantity"), sum);
                                    }
                                    else
                                    {
                                        //display "in stock" without stock quantity

...
namespace Nop.Services.Catalog
{
    ...
    public partial class CopyProductService : ICopyProductService
    {
        ...
        public Product CopyProduct(int productId, string newName, bool isPublished, bool copyImages)
        {
			...
			//using (var scope = new System.Transactions.TransactionScope(TransactionScopeOption.RequiresNew, new TransactionOptions() { IsolationLevel = IsolationLevel.ReadUncommitted, Timeout = TimeSpan.FromSeconds(30) }))
			//{
                // product
                ...
                foreach (var productVariant in productVariants)
                {
                    ...
                    var productVariantCopy = new ProductVariant()
                    {
                        ...
                        CurrencyId = productVariant.CurrencyId,
                        ...
                        DisplayPriceIfCallforPrice = productVariant.DisplayPriceIfCallforPrice,
                        HideDiscount =productVariant.HideDiscount,
                        MessageTemplateId= productVariant.MessageTemplateId,
                    ...
                    foreach (var productVariantAttribute in _productAttributeService.GetProductVariantAttributesByProductVariantId(productVariant.Id))
                    {
                        ...
                        foreach (var productVariantAttributeValue in productVariantAttributeValues)
                        {
                            var pvavCopy = new ProductVariantAttributeValue()
                            {
                                ...
                                ProductAttributeOptionId = productVariantAttributeValue.ProductAttributeOptionId
                            };
        ...
        public Product  CopyProductHideDiscountTrue(int productId, string newName, bool isPublished, bool copyImages)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
                throw new ArgumentException("No product found with the specified id", "productId");
            ...
            using (var scope = new System.Transactions.TransactionScope())
            {
                ...
                foreach (var productVariant in productVariants)
                {
                    ...
                    var productVariantCopy = new ProductVariant()
                    {
                        ...
                        CurrencyProductCost = productVariant.CurrencyProductCost,
                        DisplayPriceIfCallforPrice = productVariant.DisplayPriceIfCallforPrice,
                        HideDiscount = true, //Yeni oluşturulan ürünün variantının hideDiscount özelliği default olarak true set edildi.
                        MessageTemplateId = productVariant.MessageTemplateId,

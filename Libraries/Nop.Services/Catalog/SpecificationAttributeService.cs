...
namespace Nop.Services.Catalog
{
    ...
    public partial class SpecificationAttributeService : ISpecificationAttributeService
    {
		...
		private const string PRODUCTSPECIFICATIONATTRIBUTE_ALL = "Nop.productspecificationattribute.all";
		private const string SPECIFICATIONATTRIBUTE_ALL = "Nop.specificationattributes.all";
		private const string SPECIFICATIONATTRIBUTEOPTION_ALL = "Nop.specificationattributeoptions.all";
        #endregion
		...
		private readonly ICacheManager _cacheManagerPermanent;
            ...
            IEventPublisher eventPublisher)
        {
			...
			this._cacheManagerPermanent = EngineContext.Current.ContainerManager.Resolve<ICacheManager>("nop_cache_static");
        }
        ...
        public virtual SpecificationAttribute GetSpecificationAttributeById(int specificationAttributeId)
        {
			...
			//var query = _cacheManagerPermanent.Get(SPECIFICATIONATTRIBUTE_ALL, () =>
			//{
			//	return (IList<SpecificationAttribute>)_specificationAttributeRepository.Table.ToList();
			//});
			//query = query.Where(psa => psa.Id == specificationAttributeId).ToList();
        ...
        public virtual SpecificationAttributeOption GetSpecificationAttributeOptionById(int specificationAttributeOptionId)
        {
			...
			//var query = _cacheManagerPermanent.Get(SPECIFICATIONATTRIBUTEOPTION_ALL, () =>
			//{
			//	return (IList<SpecificationAttributeOption>)_specificationAttributeOptionRepository.Table.ToList();
			//});
			//query = query.Where(psa => psa.Id == specificationAttributeOptionId).ToList();
			//return query.FirstOrDefault();
            ...
            bool? allowFiltering, bool? showOnProductPage)
        {
			...
			//////var query = _cacheManagerPermanent.Get(PRODUCTSPECIFICATIONATTRIBUTE_ALL, () =>
			//////{
			//////	return (IList<ProductSpecificationAttribute>)_productSpecificationAttributeRepository.Table.ToList();
			//////});

			//////query = query.Where(psa => psa.ProductId == productId).ToList();
			//////if (allowFiltering.HasValue)
			//////	query = query.Where(psa => psa.AllowFiltering == allowFiltering.Value).ToList();
			//////if (showOnProductPage.HasValue)
			//////	query = query.Where(psa => psa.ShowOnProductPage == showOnProductPage.Value).ToList();
			//////query = query.OrderBy(psa => psa.DisplayOrder).ToList();

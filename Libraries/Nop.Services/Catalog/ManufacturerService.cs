...
namespace Nop.Services.Catalog
{
    ...
    public partial class ManufacturerService : IManufacturerService
    {
        ...
        private const string PRODUCTMANUFACTURERS_ALLBYMANUFACTURERID_ANDSKU_KEY = "Nop.productmanufacturer.allbymanufacturerid-{0}-{1}";
        #endregion
        ...
        /// <param name="showHidden">A value indicating whether to show hidden records</param>
        /// <returns>Product manufacturer collection</returns>
        public virtual ProductVariant GetProductVariantBySku(int manufacturerId, string sku)
        {
            if (manufacturerId == 0 || string.IsNullOrWhiteSpace(sku) || string.IsNullOrEmpty(sku))
                return null;
            ...
            try
            {
                var query = from pv in _productVariantRepository.Table
                            join pm in _productManufacturerRepository.Table on pv.ProductId equals pm.ProductId
                            where pm.ManufacturerId == manufacturerId &&
                                pv.Sku == sku
                            select pv;
                PV = query.FirstOrDefault();
            }
            catch (Exception ex)
            { 
                throw ex;

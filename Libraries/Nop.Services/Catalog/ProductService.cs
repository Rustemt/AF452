...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        {
            var query = from p in _productRepository.Table
						orderby p.DisplayOrderOnHomePage.HasValue descending, p.DisplayOrderOnHomePage, p.Name
                        where p.Published &&
                        !p.Deleted &&
                        p.ShowOnHomePage
                        select p;
        ...
        public virtual IList<Product> GetRandomRelatedProductByCategoryId(int categoryId, int count)
        {
            if (categoryId == 0)
                return new List<Product>();

            IRepository<ProductCategory> _productCategoryRepository = Nop.Core.Infrastructure.EngineContext.Current.Resolve<IRepository<ProductCategory>>();
           
            var query = (from p in _productRepository.Table
                         join pc in _productCategoryRepository.Table on p.Id equals pc.ProductId
                         where pc.CategoryId == categoryId && !p.Deleted && 
                         p.Published
                         select p).OrderBy(x => Guid.NewGuid()).Take(count);
            var relatedProducts = query.ToList();
        ...
        public virtual void DeleteProductPicture(ProductPicture productPicture)
        {
			...
			_cacheManagerPersistent.Remove(PictureRepository.NOP_CACHE_PRODUCTPICTURES);

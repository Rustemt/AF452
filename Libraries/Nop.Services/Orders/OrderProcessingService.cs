...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
            ...
            try
            {
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    //using (var scope = new System.Transactions.TransactionScope())
                    {
                        ...
                        var order = new Order()
                        {
                            ...
                            VatNumber = vatNumber,
                            CreatedOnUtc = DateTime.UtcNow,
                            Installment = processPaymentRequest.Installment == null || processPaymentRequest.Installment == 0 ? 1 : processPaymentRequest.Installment,
                            PaymentCampaignNotes = processPaymentResult.PaymentCampaignNotes
                        };
                        //AF added
                        if(!string.IsNullOrWhiteSpace(processPaymentRequest.TrackingNumber))
                        {
                            order.TrackingNumber = processPaymentRequest.TrackingNumber;
                        }
                        ...
                        int orderPlacedStoreOwnerNotificationQueuedEmailId = _workflowMessageService.SendOrderPlacedStoreOwnerNotification(order, _localizationSettings.DefaultAdminLanguageId,true);
                ...
                else
                {
                    foreach (var paymentError in processPaymentResult.Errors)
                        result.AddError(string.Format("{0}", paymentError));
                }
            }
            catch (Exception exc)
            {
        ...
        //TODO:move af folder or do not use this overriden method
        public virtual PlaceOrderResult PlaceOrder3D(ProcessPaymentRequest processPaymentRequest)
        {
            //think about moving functionality of processing recurring orders (after the initial order was placed) to ProcessNextRecurringPayment() method
            if (processPaymentRequest == null)
                throw new ArgumentNullException("processPaymentRequest");
            ...
            try
            {
                ...
                //skip payment workflow since it is already processed via 3d-pay model
                ...
                if (!skipPaymentWorkflow)
                {
                    ...
                    processPaymentRequest.PaymentMethodSystemName = "3d_pay";
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    //using (var scope = new System.Transactions.TransactionScope())
                    {
                        ...
                        var order = new Order()
                        {
                            ...
                            VatNumber = vatNumber,
                            CreatedOnUtc = DateTime.UtcNow,
                            Installment = processPaymentRequest.Installment,
                            PaymentCampaignNotes = processPaymentResult.PaymentCampaignNotes
                        };
                        _orderService.InsertOrder(order);
        ...
        //TODO:move af folder or do not use this overriden method
        public virtual decimal GetPlaceOrderTotal(ProcessPaymentRequest processPaymentRequest)
        {
            //think about moving functionality of processing recurring orders (after the initial order was placed) to ProcessNextRecurringPayment() method
            if (processPaymentRequest == null)
                throw new ArgumentNullException("processPaymentRequest");
            ...
            var result = new PlaceOrderResult();
                ...
                return processPaymentRequest.OrderTotal;
        }

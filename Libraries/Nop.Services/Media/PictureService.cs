...
namespace Nop.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        /// <summary>
        /// Gets counts of pictures
        /// </summary>
        /// <param name="pageIndex">Current page</param>
        /// <param name="pageSize">Items on each page</param>
        /// <returns>Paged list of pictures</returns>
        public virtual int GetPicturesCount()
        {
            var query = from p in _pictureRepository.Table
                        select p.Id;
        ...
        public virtual IList<Picture> GetPicturesByProductId(int productId, int recordsToReturn)
        {
			...
			return new PictureRepository().GetByProductId(productId, recordsToReturn);
			//////var pics = _cacheManager.Get("PicturesByProduct", 60, () =>
			//////{
			//////	var query = from p in _pictureRepository.Table
			//////				join pp in _productPictureRepository.Table on p.Id equals pp.PictureId
			//////				orderby pp.DisplayOrder
			//////				//where pp.ProductId == productId
			//////				select new { p.Id, p.IsNew, p.MimeType, p.NewsItemPictures, p.PictureBinary, p.ProductPictures, p.ProductVariantPictures, p.SeoFilename, pp.ProductId, pp.DisplayOrder };
			//////	return query.ToList();
			//////});            
			...
			//////pics = pics.Where(x => x.ProductId == productId).ToList();
			//////if (recordsToReturn > 0)
			//////	pics = pics.OrderBy(x => x.DisplayOrder).Take(recordsToReturn).ToList();

			//////return pics.OrderBy(x => x.DisplayOrder).Select(x => new Picture
			//////{
			//////	Id = x.Id,
			//////	IsNew = x.IsNew,
			//////	MimeType = x.MimeType,
			//////	NewsItemPictures = x.NewsItemPictures,
			//////	PictureBinary = x.PictureBinary,
			//////	ProductPictures = x.ProductPictures,
			//////	ProductVariantPictures = x.ProductVariantPictures,
        ...
        public virtual bool StoreInDb
        {
            ...
            set
            {
                ...
                if (this.StoreInDb != value)
                {
                    ...
                    int page = 0;
                    int totalPics = this.GetPicturesCount();
                    for(int x = 0; x < totalPics; x+=300)
                    {
                        var pictures = this.GetPictures(page, 300);
                        page++;

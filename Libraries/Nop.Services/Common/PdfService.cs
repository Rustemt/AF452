...
namespace Nop.Services.Common
{
    ...
    public partial class PdfService : IPdfService
    {
        ...
        public PdfService(ILocalizationService localizationService, IOrderService orderService,
            ...
            ITaxService taxService, IWorkContext workContext, ICheckoutAttributeParser checkoutAttributeParser, ICheckoutAttributeFormatter checkoutAttributeFormatter, ICheckoutAttributeService checkoutAttributeService)
        {
        ...
        protected virtual string GetLogoPath()
         {
             ...
             string logoPath = Path.Combine(_webHelper.MapPath("~/_img/"), "AlwaysFashionLogo-1.png");
             return logoPath;
        }
        #endregion
        ...
        public virtual void PrintPackagingSlipsToPdf(IList<Order> orders, string filePath)
        {
            ...
            foreach (var order in orders)
            {
                if (order.ShippingAddress != null)
                {
                    ...
                    orderTable.WidthPercentage = 100f;
                    orderTable.SetWidths(new[] { 60,40});
                    var cellHeader = new PdfPCell();
                    cellHeader.Border = Rectangle.NO_BORDER;

                    cellHeader.AddElement(new Paragraph( String.Format("{0} {1}", _localizationService.GetResource("PDFPackagingSlip.Order"), order.Id), titleFont));
                    if (!String.IsNullOrEmpty(order.ShippingAddress.Company))
                        cellHeader.AddElement(new Paragraph( String.Format(_localizationService.GetResource("PDFPackagingSlip.Company"),textInfo.ToTitleCase( order.ShippingAddress.Company.ToLower())), font));
                    cellHeader.AddElement(new Paragraph( String.Format(_localizationService.GetResource("PDFPackagingSlip.Name"), textInfo.ToTitleCase(order.ShippingAddress.FirstName.ToLower()) + " " + textInfo.ToTitleCase(order.ShippingAddress.LastName.ToLower())), font));
                    cellHeader.AddElement(new Paragraph( String.Format(_localizationService.GetResource("PDFPackagingSlip.Phone"), order.ShippingAddress.PhoneNumber), font));
                    cellHeader.AddElement(new Paragraph(String.Format(_localizationService.GetResource("PDFPackagingSlip.Address"), textInfo.ToTitleCase(order.ShippingAddress.Address1.ToLower())), font));
                    if (!String.IsNullOrEmpty(order.ShippingAddress.Address2))
                        cellHeader.AddElement(new Paragraph( String.Format(_localizationService.GetResource("PDFPackagingSlip.Address2"), textInfo.ToTitleCase(order.ShippingAddress.Address2.ToLower())), font));
                    cellHeader.AddElement(new Paragraph(String.Format("{0}, {1} {2}", textInfo.ToTitleCase(order.ShippingAddress.City.ToLower()), order.ShippingAddress.StateProvince != null ? textInfo.ToTitleCase(order.ShippingAddress.StateProvince.GetLocalized(x => x.Name).ToLower()) : "", order.ShippingAddress.ZipPostalCode != null ? order.ShippingAddress.ZipPostalCode : ""), font));
                    cellHeader.AddElement(new Paragraph( String.Format("{0}", order.ShippingAddress.Country != null ? textInfo.ToTitleCase(order.ShippingAddress.Country.GetLocalized(x => x.Name).ToLower()) : ""), font));
                    cellHeader.AddElement(new Paragraph("   ", font));
                    cellHeader.AddElement(new Paragraph(String.Format(_localizationService.GetResource("PDFPackagingSlip.ShippingMethod"), order.ShippingMethod), font));
                    cellHeader.AddElement(new Paragraph("   ", font));
                    orderTable.AddCell(cellHeader);

                    // AlwaysFashion Logo
                    string pictureUrl = GetLogoPath();
                    var logoFile = Image.GetInstance(pictureUrl);
                    logoFile.ScaleToFit(202, 60);
                    var cellLogo = new PdfPCell(logoFile);
                    cellLogo.Border = Rectangle.NO_BORDER;
                    orderTable.AddCell(cellLogo);
        ...
        public void PrintOrdersSalesAggrimentToPdf(string html, int orderid)
        {
            ...
            PdfWriter.GetInstance(document, new FileStream(filePath + "\\SalesAgreement-" + orderid + ".pdf", FileMode.Create));
            document.Open();


            HTMLWorker hw = new HTMLWorker(document);
            FontFactory.Register(Path.Combine(_webHelper.MapPath("~/App_Data/Pdf/arial.ttf")), "Garamond");
            StyleSheet css = new StyleSheet();
            css.LoadTagStyle("body", "face", "Garamond");
            css.LoadTagStyle("body", "encoding", "Identity-H");
            css.LoadTagStyle("body", "size", "12pt");

            css.LoadTagStyle("th", "face", "Garamond");
            css.LoadTagStyle("th", "encoding", "Identity-H");
            css.LoadTagStyle("th", "size", "12pt");

            css.LoadTagStyle("td", "face", "Garamond");
            css.LoadTagStyle("td", "encoding", "Identity-H");
            css.LoadTagStyle("td", "size", "12pt");

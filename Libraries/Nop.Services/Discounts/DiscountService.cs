    ...
    public partial class DiscountService : IDiscountService
    {
        ...
        public virtual Discount GetDiscountByCouponCode(string couponCode, bool showHidden = false)
        {
            ...
            var discount = discounts.
                Where(d => !String.IsNullOrEmpty(d.CouponCode) && IsDiscountCouponCodeValid(d, couponCode))
                .FirstOrDefault();
            return discount;
        }
            ...
            int? customerId, int pageIndex, int pageSize)
        {
			...
			var duhList = _cacheManagerPersistent.Get("DiscountUsageHistory", 10, () =>
			{
				return _discountUsageHistoryRepository.Table.ToList();
			});

            //var query = _discountUsageHistoryRepository.Table;
			var query = duhList.Where(duh => true);
            if (discountId.HasValue && discountId.Value > 0)
				query = duhList.Where(duh => duh.DiscountId == discountId.Value);

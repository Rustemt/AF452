        ...
        public void Execute()
        {
            ...
            foreach (var queuedEmail in queuedEmails)
            {
                ...
                catch (Exception exc)
                {
                    ...
                    {
                        _logger.Error(string.Format("Error sending e-mail inner . {0}", exc.InnerException.ToString()));

                        if(exc.InnerException.InnerException!=null)

                            _logger.Error(string.Format("Error sending e-mail inner_inner . {0}", exc.InnerException.InnerException.ToString()));
                    }
                }
                finally
                {

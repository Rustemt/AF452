   ...
   // InitAttributeSelections();
    ...
    $(window).scroll(function () {
        if ($(window).scrollTop() + 150 > $(document).height() - $(window).height()) {
            ...
            if ($('input[type="hidden"]#hasMore').val().toLowerCase() == 'true' && _loadedWithAjax != null && _loadedWithAjax)
...
function SearchAndFilterProducts(updatePrice, trigger) {
    ...
    for (var i = 0; i < filterItems.length; i++) {
        if ($(filterItems[i]).attr('rel') != null) {
            ...
            if (value.substring(0, 4) === 'item') {
                specifications.push($(filterItems[i]).attr('rel').substring(4));
            }
            else if (value.substring(0, 5) === 'brand') {
                manufacturers.push($(filterItems[i]).attr('rel').substring(5));
            }
                ...
                categories.push($(filterItems[i]).attr('rel').substring(3));
            }
        }
    }
    ...
    context = { categoryId: $('input[type="hidden"]#categoryId').val(),
        ...
        PriceRangeFilter : { Min: "0", Max: $('div.priceSliderValue strong').text(), From:0, To: $('input#slider-Value').val() },
        SpecificationFilter: { FilteredOptionIds: specifications },
        //ProductAttributeFilter: { AlreadyFilteredItems: attributes },
        ManufacturerFilter: { FilteredManufacturerIds: manufacturers },
        CategoryFilter: { FilteredCategoryIds: categories },
    ...
    queryString = "?categoryId=" + (context.categoryId != null ? context.categoryId : "")
        + "&ViewMode="  + context.ViewMode
        + "&OrderBy="  + context.OrderBy
        + "&PriceRangeFilter.Min="  + context.PriceRangeFilter.Min
        + "&PriceRangeFilter.Max="  + context.PriceRangeFilter.Max
        + "&PriceRangeFilter.From="  + context.PriceRangeFilter.From
        + "&PriceRangeFilter.To="  + context.PriceRangeFilter.To
        + "&SpecificationFilter.FilteredOptionIds=" + context.SpecificationFilter.FilteredOptionIds.join()
        + "&ManufacturerFilter.FilteredManufacturerIds=" + context.ManufacturerFilter.FilteredManufacturerIds.join()
        + "&CategoryFilter.FilteredCategoryIds=" + context.CategoryFilter.FilteredCategoryIds.join()
        + "&PageSize="  + context.PageSize
        + "&PageNumber="  + context.PageNumber
        + "&FilterTrigging="  + context.FilterTrigging
        + "&Q="  + context.Q;
    ...
    window.location = location.protocol + "//" + location.hostname + ":" + location.port + location.pathname + queryString;

    //  $('body').append('<div id="loading" style="position:fixed; top:0px;left:0px; width:100%; height:100%; z-index:1000; opacity:0.70; background: url(/_img/loading.gif) #f3f3f3 center center no-repeat; overflow:hidden; "></div>');
    /*
    if ($('.panel').length < _totalProducts) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (trigger == 'C' || trigger == 'M' || trigger == 'S' || trigger == 'A') {
                    ...
                    if ($('div#content div.yourSelection ul.selections li').length > 1) {
                        $('div#content div.yourSelection ul.selections').show();
                    }
                } else if (trigger == 'P') {
                    //$('div#content div.yourSelection ul.selections').html(content);
                }
...
//    $('div#content div.yourSelection div.selection div.selectionMenu div.selectionMenuInner div.item:not(#C,#M)').each(function () {
//        var filtered = $(this).find('a.checked').length > 0;
//        if (filtered) {
//            $(this).find('a:not(.checked)').addClass('dissel');
//            $(this).find('a:not(.checked)').unbind("click");
//            $(this).find('a:not(.checked)').bind("click", function () { return false; });
//        }

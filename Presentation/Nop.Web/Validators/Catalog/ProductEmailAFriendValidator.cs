...
namespace Nop.Web.Validators.Catalog
{
    public class ProductEmailAFriendValidator : AbstractValidator<ProductEmailAFriendModel>
    {
        public ProductEmailAFriendValidator(ILocalizationService localizationService)
        {
            ...
            RuleFor(x => x.FriendName).NotEmpty().WithMessage(localizationService.GetResource("Products.EmailAFriend.FriendName.Required"));
            RuleFor(x => x.YourName).NotEmpty().WithMessage(localizationService.GetResource("Products.EmailAFriend.YourName.Required"));

            RuleFor(x => x.PersonalMessage).NotEmpty().WithMessage(localizationService.GetResource("Products.EmailAFriend.Explanation.Required"));
        }}
}
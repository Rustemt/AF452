            ...
            if (discount != null)
            {
                ...
                switch (discount.DiscountType)
                {
                    case DiscountType.AssignedToManufacturers:
                        relatedItemIds = string.Join(",", discount.AppliedToManufacturers.Select(x => x.Id));
                        break;
                    case DiscountType.AssignedToSkus:
                        relatedItemIds = string.Join(",", discount.AppliedToProductVariants.Select(x => x.Id));
                        break;
                    case DiscountType.AssignedToCategories:
                        relatedItemIds = string.Join(",", discount.AppliedToCategories.Select(x => x.Id));
                        break;
                }
                model.RelatedItemIds = relatedItemIds;

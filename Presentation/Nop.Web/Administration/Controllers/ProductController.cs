...
namespace Nop.Admin.Controllers
{
    ...
    public class ProductController : BaseNopController
    {
        ...
        private void FirstVariant_PrepareProductVariantModel(ProductVariantModel model, ProductVariant variant, bool setPredefinedValues)
        {
            ...
            //message templates
            var messageTemplates = _messageTemplateService.GetAllMessageTemplates().Where(mt => mt.Name.Contains("PriceOffer.CustomerNotification"));
            
            foreach (var mt in messageTemplates)
                model.MessageTemplates.Add(new SelectListItem() { Text = mt.Name, Value = mt.Id.ToString(), Selected = variant != null });
            ...
            PrepareCurrencyModel(model, variant);
        ...
        //AF
        [NonAction]
        private void PrepareCurrencyModel(ProductVariantModel model, ProductVariant variant)
        {
            var availableCurrencies = _currencyService.GetAllCurrencies().Select(x => x.ToModel()).ToList();
            model.Currencies = availableCurrencies.Select(x => new SelectListItem() { Text = x.Name, Value = x.Id.ToString(), Selected = (variant != null && variant.CurrencyId.HasValue && variant.CurrencyId == x.Id) }).ToList();
            if (model.Currencies.FirstOrDefault(x => x.Selected) == null)
                model.Currencies.FirstOrDefault(x => x.Value == _currencySettings.PrimaryStoreCurrencyId.ToString()).Selected = true;
        }
        ...
        public ActionResult List()
        {
            ...
            PagedList<Product> products = new PagedList<Product>(new List<Product>(), 0, 1);
            ProductListModel productListModel = new ProductListModel();
            productListModel.DisplayProductPictures = this._adminAreaSettings.DisplayProductPictures;
            GridModel<ProductModel> gridModel = new GridModel<ProductModel>();
            gridModel.Data = products.Select<Product, ProductModel>((Product x) =>
            {
                ProductModel model = x.ToModel();
                this.PrepareProductPictureThumbnailModel(model, x);
                this.PrepareVariantsModel(model, x);
                return model;
            });
            gridModel.Total = products.TotalCount;
            productListModel.Products = gridModel;
            IList<SelectListItem> availableCategories = productListModel.AvailableCategories;
            SelectListItem selectListItem = new SelectListItem();
            selectListItem.Text = this._localizationService.GetResource("Admin.Common.All");
            selectListItem.Value = "0";
            availableCategories.Add(selectListItem);
            foreach (Category allCategory in this._categoryService.GetAllCategories(true))
            {
                IList<SelectListItem> selectListItems = productListModel.AvailableCategories;
                SelectListItem categoryNameWithPrefix = new SelectListItem();
                categoryNameWithPrefix.Text = allCategory.GetCategoryNameWithPrefix(this._categoryService);
                categoryNameWithPrefix.Value = allCategory.Id.ToString();
                selectListItems.Add(categoryNameWithPrefix);
            }
            IList<SelectListItem> availableManufacturers = productListModel.AvailableManufacturers;
            SelectListItem resource = new SelectListItem();
            resource.Text = this._localizationService.GetResource("Admin.Common.All");
            resource.Value = "0";
            availableManufacturers.Add(resource);
            foreach (Manufacturer allManufacturer in this._manufacturerService.GetAllManufacturers(true))
            {
                IList<SelectListItem> availableManufacturers1 = productListModel.AvailableManufacturers;
                SelectListItem name = new SelectListItem();
                name.Text = allManufacturer.Name;
                name.Value = allManufacturer.Id.ToString();
                availableManufacturers1.Add(name);
            }
            return base.View(productListModel);
           ...
           // //    ProductSortingEnum.CreatedOn, 0, _adminAreaSettings.GridPageSize, true);
        ...
        public ActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //af currency price
                if (variant.CurrencyId.HasValue && (variant.CurrencyId.Value != _currencySettings.PrimaryStoreCurrencyId))
                {
                    var cur = _currencyService.GetCurrencyById(variant.CurrencyId.Value);
                    variant.CurrencyPrice = variant.Price;
                    variant.Price = _currencyService.ConvertToPrimaryStoreCurrency(variant.Price, cur);
                    variant.CurrencyOldPrice = variant.OldPrice;
                    variant.OldPrice = _currencyService.ConvertToPrimaryStoreCurrency(variant.OldPrice, cur);
                    variant.CurrencyProductCost = variant.ProductCost;
                    variant.ProductCost = _currencyService.ConvertToPrimaryStoreCurrency(variant.ProductCost, cur);
                }
        ...
        public ActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            Product productById = this._productService.GetProductById(model.Id);
            if (productById == null || productById.Deleted)
            {
                return base.RedirectToAction("List");
            }
            if (!base.ModelState.IsValid)
            {
                this.PrepareTags(model, productById);
                this.PrepareCopyProductModel(model, productById);
                this.PrepareVariantsModel(model, productById);
                this.PrepareTemplatesModel(model);
                this.PrepareAddSpecificationAttributeModel(model);
                this.PrepareAddProductPictureModel(model);
                this.PrepareCategoryMapping(model);
                this.PrepareManufacturerMapping(model);
                return base.View(model);
            }
            productById = model.ToEntity(productById);
            productById.UpdatedOnUtc = DateTime.UtcNow;
            this._productService.UpdateProduct(productById);
            this.UpdateLocales(productById, model);
            this.SaveProductTags(productById, this.ParseProductTags(model.ProductTags));
            this.UpdatePictureSeoNames(productById);
            ICustomerActivityService customerActivityService = this._customerActivityService;
            string resource = this._localizationService.GetResource("ActivityLog.EditProduct");
            object[] name = new object[] { productById.Name };
            customerActivityService.InsertActivity("EditProduct", resource, name);
            this.SuccessNotification(this._localizationService.GetResource("Admin.Catalog.Products.Updated"), true);
            if (!continueEditing)
            {
                return base.RedirectToAction("List");
            }
            return base.RedirectToAction("Edit", new { id = productById.Id });
        ...
        public ActionResult RelatedProductAddPopupList(GridCommand command, ProductModel.AddRelatedProductModel model)
        {
            ...
            {
                products = _productService.SearchProducts(model.SearchCategoryId, model.SearchManufacturerId,
        ...
        public ActionResult RetargetingExportXml()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();
        ...
        public ActionResult ExportExcelDefaultLanguage()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();
            ...
            try
            {
                ...
                _exportManager.ExportProductsToXlsxWithDefaultLanguage(filePath, products);
        ...
        public ActionResult ExportExcelDefaultLanguageSelected(string selectedIds)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();
        ...
        public ActionResult ExportExcelForNebim()
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();
        ...
        public ActionResult ExportExcelForNebimSelected(string selectedIds)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();
        ...
        [HttpPost]
        public ActionResult ImportXml(FormCollection form)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();
                ...
                var file = Request.Files["importxmlfile"];
                    ...
                    if (file != null && file.ContentLength > 0)
                    {
                        if (file.ContentType == "text/xml")
                        {
                            ...
                            file.InputStream.Read(fileBytes, 0, file.ContentLength);
                            //do stuff with the bytes
                            string fileName = string.Format("products_{0}_{1}.xml",
                                                            DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss"),
                                                            CommonHelper.GenerateRandomDigitCode(4));
                            string filePath = string.Format("{0}content\\files\\ExportImport\\{1}",
                                                            Request.PhysicalApplicationPath, fileName);

                            System.IO.File.WriteAllBytes(filePath, fileBytes);
                            _importManager.UpdateProductVariantsFromPirlantXml(filePath);
                        }
                        else
                        {
                            ErrorNotification("Please upload an text/xml file");
                            return RedirectToAction("List");      
                        }
                    }
                    else
...
//        //--open-- KPCHANGE July 22, 2013
//        private readonly ISpecificationAttributeService _specificationAttributeService;
//        //--closed-- KPCHANGE July 22, 2013

//        //--open-- KPCHANGE July 22, 2013 - added , ISpecificationAttributeService specificationAttributeService to the end of this method
...
//            IUrlRecordService urlRecordService, ISpecificationAttributeService specificationAttributeService)
//        //--closed-- KPCHANGE July 22, 2013
//        {
//            this._productService = productService;
//            this._categoryService = categoryService;
...
//                    //--open------------- KPCHANGE - July 22, 2013 --- modifying import code to import a products variants and its specifications
//                    string specifications = worksheet.Cells[iRow, GetColumnIndex(properties, "Specifications")].Value as string;
//                    //--closed
...
//                        //are there any specifications for this product?
//                        if (specifications != null) {
                            
//                            XmlDocument doc = new XmlDocument();
//                            doc.LoadXml(specifications);
                             
//                            XmlNodeList _ProductSpecificationAttributes = doc.GetElementsByTagName("psa");
 
//                            foreach (XmlNode node in _ProductSpecificationAttributes)
//                            {
//                                //product specification attribute element
//                                XmlElement psaElement = (XmlElement) node;
 
//                                var psa = new ProductSpecificationAttribute()
//                                {
//                                    ProductId = product.Id,
//                                    Id = Convert.ToInt32(psaElement.GetElementsByTagName("id")[0].InnerText),
//                                    SpecificationAttributeOptionId = Convert.ToInt32(psaElement.GetElementsByTagName("saoid")[0].InnerText),
//                                    CustomValue = psaElement.GetElementsByTagName("cv")[0].InnerText,
//                                    AllowFiltering = Convert.ToBoolean(psaElement.GetElementsByTagName("af")[0].InnerText),
//                                    ShowOnProductPage = Convert.ToBoolean(psaElement.GetElementsByTagName("sopp")[0].InnerText),
//                                    DisplayOrder = Convert.ToInt32(psaElement.GetElementsByTagName("do")[0].InnerText),
//                                };

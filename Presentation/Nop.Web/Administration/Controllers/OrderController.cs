...
namespace Nop.Admin.Controllers
{
	...
	public class OrderController : BaseNopController
    {
        ...
        public OrderController(IOrderService orderService, 
            ...
            MeasureSettings measureSettings, PdfSettings pdfSettings, ITopicService topicService, IMessageTokenProvider messageTokenProvider, ITokenizer tokenizer)
		{
        ...
        private void PrepareOrderDetailsModel(OrderModel model, Order order)
        {
            ...
            //AF 
            model.Installment = order.Installment.HasValue ? order.Installment.ToString() : string.Empty;
            model.PaymentCampaignNotes = order.PaymentCampaignNotes;
            ...
            #region Discounts
            if(order.DiscountUsageHistory!=null)
            {
              model.Discounts = string.Join(",", order.DiscountUsageHistory.Select(n => n.Discount.Name));
            }
        ...
        public ActionResult MarkOrderAsPaid(int id)
        {
            ...
            try
            {
                ...
                // send order to nebim
                // return RedirectToAction("SendOrderToNebim", "ExternalIntegration", new { orderId = order.Id });
                var model = new OrderModel();
                PrepareOrderDetailsModel(model, order);
                return View(model);
            }
        ...
        public ActionResult PdfInvoice(int orderId)
        {
            ...
            _pdfService.PrintInvoiceToPdf(orders, _workContext.WorkingLanguage, filePath);
            var bytes = System.IO.File.ReadAllBytes(filePath);
        ...
        public ActionResult SalesAgrrementPdf(int orderId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageOrders))
                return AccessDeniedView();

            var path = HostingEnvironment.MapPath("~/Content/Pdf/");
            var fullpath = path + "\\SalesAgreement-" + orderId + ".pdf";
            ...
            if (!System.IO.File.Exists(fullpath))
            {
                ...
                string html = _tokenizer.Replace(c.Body, tokens, true);
                string bodyReplaced = _tokenizer.Replace(html, tokens, true);
                int order_id = orderId;

                _pdfService.PrintOrdersSalesAggrimentToPdf(bodyReplaced, order_id);

            }

            string filePath = HostingEnvironment.MapPath("~/Content/Pdf/");
            var p = filePath + "\\SalesAgreement-" + orderId + ".pdf";

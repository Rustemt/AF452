...
namespace Nop.Admin.Controllers
{
    ...
    public class ProductVariantController : BaseNopController
    {
        ...
        public ProductVariantController(IProductService productService, IPictureService pictureService,
            ...
            AdminAreaSettings adminAreaSettings, IMessageTemplateService messageTemplateService)
        {
        ...
        //AF
        [NonAction]
        private void UpdateAttributeValueLocales(ProductVariantAttributeValue pvav, ProductAttributeOptionModel model)
        {
        ...
        private void PrepareProductVariantModel(ProductVariantModel model, ProductVariant variant, bool setPredefinedValues)
        {
            ...
            var messageTemplates =
                _messageTemplateService.GetAllMessageTemplates().Where(
                    n => n.Name.Contains("PriceOffer.CustomerNotification"));
            ...
            if (variant != null && variant.CurrencyId.HasValue && (variant.CurrencyId.Value != _currencySettings.PrimaryStoreCurrencyId))
            {
                model.Price = variant.CurrencyPrice.HasValue ? variant.CurrencyPrice.Value : model.Price;
                model.OldPrice = variant.CurrencyOldPrice.HasValue ? variant.CurrencyOldPrice.Value : variant.OldPrice;
                model.ProductCost = variant.CurrencyProductCost.HasValue ? variant.CurrencyProductCost.Value : variant.ProductCost;
                model.MessageTemplateId = variant.MessageTemplateId;
            }
        ...
        public ActionResult Edit(int id)
        {
            ...
            PrepareAddProductVariantPictureModel(model);
            return View(model);
        }
        ...
        public ActionResult ProductAttributeValueCreatePopup(int productAttributeAttributeId)
        {
            ...
            //AF
            var productVariantAttribute = _productAttributeService.GetProductVariantAttributeById(productAttributeAttributeId);
            var productAttributeOptions = _productAttributeService.GetProductAttributeOptionsByProductAttribute(productVariantAttribute.ProductAttributeId);
            var productAttributeOptionModels = productAttributeOptions.Select(x => x.ToModel()).ToList();
            ...
            //locales
            AddLocales(_languageService, model.Locales);
            model.ProductAttributeOptionModels = productAttributeOptionModels;
            return View(model);
        }
        ...
        public ActionResult ProductAttributeValueCreatePopup(string btnId, string formId, ProductVariantModel.ProductVariantAttributeValueModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ProductVariantAttributeValue pvav = null;
                if (model.ProductAttributeOptionId == 0)
                {
                    pvav = new ProductVariantAttributeValue()
                    {
                ...
                {
                    var option = _productAttributeService.GetProductAttributeOptionById(model.ProductAttributeOptionId);
                    var optionModel = option.ToModel();
                    AddLocales(_languageService, optionModel.Locales, (locale, languageId) =>
                    {
                    ...
                    _productAttributeService.InsertProductVariantAttributeValue(pvav);
                    UpdateAttributeValueLocales(pvav, optionModel);
                }
        ...
        public ActionResult ProductAttributeValueEditPopup(int id)
        {
            ...
            var productAttributeOptions = _productAttributeService.GetProductAttributeOptionsByProductAttribute(pvav.ProductVariantAttribute.ProductAttributeId);
            var productAttributeOptionModels = productAttributeOptions.Select(x => x.ToModel()).ToList();
            model.ProductAttributeOptionModels = productAttributeOptionModels;
        ...
        public ActionResult ProductAttributeValueEditPopup(string btnId, string formId, ProductVariantModel.ProductVariantAttributeValueModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    pvav.Name = option.Name;
                    pvav.ProductAttributeOptionId = option.Id;
                    pvav.PriceAdjustment = model.PriceAdjustment;
                    pvav.WeightAdjustment = model.WeightAdjustment;
                    pvav.IsPreSelected = model.IsPreSelected;
                    pvav.DisplayOrder = model.DisplayOrder;
            ...
            var productVariantAttributesModel = productVariantAttributeCombinations
                .Select(x =>
                {
                    var pvacModel = new ProductVariantModel.ProductVariantAttributeCombinationModel()
                    {
                        ...
                        ProductVariantBarcode=x.ProductVariantBarcode,
        ...
        public ActionResult ProductVariantAttrbiuteCombinationUpdate(GridCommand command, ProductVariantModel.ProductVariantAttributeCombinationModel model)
        {
            ...
            pvac.ProductVariantBarcode = model.ProductVariantBarcode;
            ...
            AddProductVariantAttributeCombinationModel model, FormCollection form)
        {
            ...
            string attributes = "";
            string productVariantBarcode = model.ProductVariantBarcode;
        ...
        #region Product variant pictures

        public ActionResult ProductVariantPictureAdd(int pictureId, int displayOrder, int productVariantId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();
            ...
            _productService.InsertProductVariantPicture(new ProductVariantPicture()
            {
                PictureId = pictureId,
        ...
        [HttpPost, GridAction(EnableCustomBinding = true)]
        public ActionResult ProductVariantPictureList(GridCommand command, int productVariantId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();

            var productVariantPictures = _productService.GetProductVariantPicturesByProductVariantId(productVariantId);
            var productVariantPicturesModel = productVariantPictures
                .Select(x =>
                {
                    return new ProductVariantModel.ProductVariantPictureModel()
                    {
                        Id = x.Id,
                        ProductVariantId = x.ProductVariantId,
            ...
            var model = new GridModel<ProductVariantModel.ProductVariantPictureModel>
            {
                Data = productVariantPicturesModel,
                Total = productVariantPicturesModel.Count
            };
        ...
        [GridAction(EnableCustomBinding = true)]
        public ActionResult ProductVariantPictureUpdate(ProductVariantModel.ProductVariantPictureModel model, GridCommand command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();
            ...
            var productVariantPicture = _productService.GetProductVariantPictureById(model.Id);
            if (productVariantPicture == null)
                throw new ArgumentException("No product variant picture found with the specified id");

            productVariantPicture.DisplayOrder = model.DisplayOrder;
            _productService.UpdateProductVariantPicture(productVariantPicture);

            return ProductVariantPictureList(command, productVariantPicture.ProductVariantId);
        }

        [GridAction(EnableCustomBinding = true)]
        public ActionResult ProductVariantPictureDelete(int id, GridCommand command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();

            var productVariantPicture = _productService.GetProductVariantPictureById(id);
            if (productVariantPicture == null)
                throw new ArgumentException("No product variant picture found with the specified id");

            var productVariantId = productVariantPicture.ProductVariantId;
            _productService.DeleteProductVariantPicture(productVariantPicture);

            return ProductVariantPictureList(command, productVariantId);
        }

        [NonAction]
        private void PrepareAddProductVariantPictureModel(ProductVariantModel model)
        {
            if (model == null)
                throw new ArgumentNullException("model");

            if (model.AddPictureModel == null)
                model.AddPictureModel = new ProductVariantModel.ProductVariantPictureModel();
        }
        #endregion
